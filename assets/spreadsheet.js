(async function(){
  ...
  function positionFilterPanel(toggle, panel){
    ...
    const desiredLeft = toggleRect.left - controlsRect.left;
  ...
  function render(){
    ...
    for(const m of filteredNormal){
      ...
      tr.appendChild(td(
        modeLabel,
        `nowrap cell-mode ${cellModeClass(m.mode)}`
      ));
  ...
  els.q.addEventListener('input', debounce(render));
