(async function(){
  ...
  // Normalize seasonal to match your site expectations
  const SEASON_ORDER = [
    'Lunar New Year',
    'Easter',
    'Summer',
  ...
  function stripLeadingEmoji(name){
    ...
    // Remove a single leading emoji + optional space
    // (Works in modern browsers; safe fallback returns original)
    try{
  ...
  function detectSeasonLabel(m){
    // Prefer explicit field(s) if your seasonal JSON includes them
    const direct =
      m.season ||
      m.seasonLabel ||
      m.event ||
    ...
    // Fallback: infer from a leading emoji in the name
  ...
  // combine base maps + seasonal maps ONCE
  ...
  const USE_REAL_TODAY = true; // set to false to anchor to meta.latest_rotation
  ...
  function parseDateLoose(s){
    ...
    // Expecting "January 11, 2026" style from your builder
    const dt = new Date(t);
  ...
  function mapCard(m){
    ...
    // bottom overlay (title on left, date + days on right)
    const overlay = document.createElement('div');
    overlay.className = 'mapOverlay';

    // left side (map name only)
  ...
  function render(){
    ...
    function daysVal(m){
      ...
      // Unknown should behave like "infinite days"
    ...
    function seasonalFirstOrLast(a,b){
      if(!!a.isSeasonal !== !!b.isSeasonal){
        if(seasonalActive){
          return a.isSeasonal ? -1 : 1; // active seasonal goes first
        }
        return a.isSeasonal ? 1 : -1;  // inactive seasonal goes last
      }
      return 0;
    }
    ...
    function seasonalInternalSort(a,b, sortKey){
      ...
      // Within each season: alphabetical by map name (respect A->Z / Z->A)
    ...
    function compare(a,b){
      ...
      // NAME sorts:
      // - seasonal block goes top if active, bottom if inactive
      // - seasonal block ordered by seasonOrder, then alphabetical
      if(!isDaysSort){
      ...
      // DAYS sorts:
      // - seasonal maps mix with normal maps by days (no seasonal block rules)
      // - Unknown counts as Infinity:
      //    low->high => last, high->low => first
      const da = daysVal(a);
      const db = daysVal(b);
    ...
    // keep In/Out separated, but sort within each section
  ...
  function seasonalSortKey(m, seasonalActive){
    // If seasonal is active: seasonal maps first (0), normal after (1)
    // If seasonal is NOT active: normal first (0), seasonal after (1)
    if(seasonalActive){
